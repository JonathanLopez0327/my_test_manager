...
describe("POST /api/test-runs/[id]/items/[runItemId]/executions/[executionId]/complete", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("returns 409 when run is completed", async () => {
    const response = await POST(
      new Request("http://localhost/api/test-runs/run-1/items/item-1/executions/exec-1/complete", {
        method: "POST",
