...
import { reinstateLicense } from "@/lib/keygen/client";
...
export const POST = withAuth(null, async (_req, { globalRoles }, { params }) => {
  ...
  try {
    const state = await reinstateLicense(org.keygenLicenseId);
    ...
    console.error("[keygen] reinstateLicense failed:", err);
    return NextResponse.json(
      { message: "Could not reinstate license via Keygen." },
      { status: 502 },
    );
  }
});
