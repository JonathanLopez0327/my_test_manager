...
export function TestRunDetailsModal({
  open,
  run,
  onClose,
  canManage = true,
  onUpdated,
}: TestRunDetailsModalProps) {
      ...
      <div className="space-y-4">
        ...
        {!loading && !error && tab === "artifacts" ? (
          <div className="space-y-3">
              ...
              <div className="rounded-lg border border-stroke bg-white p-4">
                ...
                <div className="mt-4 flex justify-end">
                  <button
                    ...
                    className="rounded-full bg-brand-600 px-4 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:bg-brand-200"
                  >
                    {savingArtifact ? "Guardando..." : "Agregar artefacto"}
                  </button>
                </div>
              </div>
            ) : null}
            {artifacts.length === 0 ? (
              <div className="rounded-lg border border-dashed border-stroke px-4 py-6 text-sm text-ink-muted">
                No hay artefactos para este run.
              </div>
            ) : (
              <div className="space-y-3">
                {artifacts.map((artifact) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-ink">
                          {artifact.name ?? "Artefacto sin nombre"}
                        </p>
                        <p className="text-xs text-ink-muted">
                          {artifact.type.toUpperCase()} ·{" "}
                          {artifact.mimeType ?? "Sin mime"}
                        </p>
                      </div>
                      <a
                        href={artifact.url}
                    ...
                    <div className="mt-3 text-xs text-ink-muted">
                      <p>Item: {artifact.runItemId ?? "Run completo"}</p>
                      <p>Creado: {formatDate(artifact.createdAt)}</p>
                      {artifact.checksumSha256 ? (
                        <p className="mt-2 text-ink-soft">
                          SHA256: {artifact.checksumSha256.slice(0, 12)}...
                        </p>
                      ) : null}
                    </div>
                  </div>
