...
1. Configure environment variables (see `.env.example`):
   - Required in runtime: `DATABASE_URL`, `NEXTAUTH_SECRET`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `S3_ENDPOINT`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`, `S3_REGION`, `S3_BUCKET`, `S3_PUBLIC_URL`, `LANGGRAPH_API_URL`, `LANGGRAPH_QA_ID`, `LANGGRAPH_API_KEY`, `AI_AGENT_TOKEN_ENCRYPTION_KEY`
   - Seed-only variables: `SEED_QA_EMAIL`, `SEED_QA_PASSWORD`, `SEED_SUPER_ADMIN_EMAIL`, `SEED_SUPER_ADMIN_PASSWORD`
   - Optional controls: `AI_AGENT_TOKEN_TTL_DAYS`, `AI_AGENT_TOKEN_ROTATE_BEFORE_DAYS` (when `LANGGRAPH_API_KEY` is missing, chat falls back only in development and fails in production).
